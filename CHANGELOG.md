...
- 建立面向土壤科学及相关自然科学的中英文写作、翻译、润色、术语修复、图表分析和正式文件交付流程。
- 整合并专业化人类自然写作审查，将自然度置于事实守恒、证据强度、学科语域和文体功能之后，不以AI检测器规避作为可承诺指标。
- 覆盖29类文体路由，配置相应的语言功能、长度控制、官方模板、封面、字号名称、行距、首行缩进及DOCX/PDF/PPTX渲染验收。
- 形成图表用户控制合同、证据清单、可视化类型边界、中文盲审模板及44个图表审计案例，区分图注、结果描述与解释性分析。
- 建立土壤学语域词库、中英双语对齐和翻译干扰审计，控制管理化、宣传化、空泛和机械直译表达。
- 建立文献元数据索引、授权全文表达生产管线、SHA-256锨定、双人复核和D1–D13缺口报告，防止将元数据数量写成全文或合格表达数量。
- 建立OpenAI、Claude、Gemini、DeepSeek、Qwen、Mistral、Cohere、Amazon Bedrock、Ollama和自定义接口的协议适配器、严格响应归一化及离线资格边界。
- 通过162项确定性回归测试和技能包结构校验。

发布包：`dist/soil-all-writing/soil-all-writing-skill-20260817-v1.zip`
